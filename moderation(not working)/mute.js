...
client.counts = require("../counts.json");
    ...
    client.counts = require("../counts.json");
    let casenum = bot.counts["casecount"];
    ...
    fs.writeFile("./pendingmutes.json", JSON.stringify(bot.pendingmutes, 2, null), (err) => {
    ...
    message.channel.messages.fetch({limit: 100}).then(messages => {
        ...
        if (membermsgs.length > 0 && !["dm", "pfp", "avatar", "username", "profile picture", " vc", " vc ", "voice channel"].some(rV => mReason.toLowerCase().includes(rV))) {
            ...
            SourceBin.newBin(`Case ${casenum} - ${mUser.tag}/${mUser.id}\nReason: ${mReason}\n\nLAST MESSAGES:\n\n${msgsArray.join(`\n`)}`, 'txt', 'wrapper', {
                title: "",
                description: ""
    ...
    bot.counts["casecount"]++
    fs.writeFile("./counts.json", JSON.stringify(bot.counts), (err) => {
      if (err) console.log(err);
    });
