...
module.exports = {
    ...
    .addField("Aliases", `\`${prefix}ban\``, true)
    .addField("Allowed Roles", `${allowedRoles.map(rID => `<@&${rID}>`).join(' | ')}`, true)
    .addField("Usage", `\`${prefix}ban\``)
    .addField("Example", `\`${prefix}ban @joey NSFW\``)
    ...
    .setColor("#39fa53")
    .setAuthor('User Banned | ' + message.guild.name, message.guild.iconURL())
.setDescription(`<:NB_IconPinUnread:790265429691072542> \`${bUsertag}\` has been banned from Night Bar.\n**Moderator:** <@${message.author.id}>`)
