...
aliases: ['setsc', 'setstrikeroom'],
description: "set ur guild  join/leave channels",
utilisation: `{prefix}setstrikechannel`,
category: "Strike",
...
async execute(client, message, args, member) {
  ...
  if(!message.member.hasPermission('ADMINISTRATOR')) {
    ...
    1️⃣ Strike
    2️⃣ View Current Strike channel
    ❌ Cancel
    `)
    .setColor(`2C2F33`)
...
.setTitle(' ')
.setDescription(`
Sucssesfully Updated the Strike Channel`)
.setColor(`2C2F33`)
.setFooter( message.guild.name , client.user.displayAvatarURL())
.setTimestamp()
 
await react.edit(changed)
await db.set(`strike_channel_${message.guild.id}`, channel.id)
await join.stop()
})
    }   
...
.setDescription(`
Join channel => 
${message.guild.channels.cache.get(db.get(`strike_channel_${message.guild.id}`)) || "None"}
`)
.setColor(`2C2F33`) 
.setFooter( message.guild.name , client.user.displayAvatarURL())
.setTimestamp()
