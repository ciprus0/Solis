...
module.exports = {
    name: 'walletleaderboard',
    aliases: ['walletlb', 'wlb'],
    category: 'User',
    utilisation: '{prefix}walletlb <page>',
    module: 'economy',
    ...
    let data
    try {
        await db.promise().query('SELECT * FROM user ORDER BY pocket DESC', [message.guild.id]).then(result => {
            data = result[0]
        })
    } catch (error) {
        console.log(error)
    ...
    .setAuthor(`Global Leaderboard | Wallet NightBux`,message.guild.iconURL())
    .setColor("#f44336")
    .setDescription(page.map(e => `<@${e.user_id}> | <:wallet:902400881029431348> Wallet **${e.pocket}**`))
