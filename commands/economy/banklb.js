...
module.exports = {
    ...
    aliases: ['banklb', 'blb'],
    category: 'User',
    utilisation: '{prefix}banklb <page>',
    module: 'economy',
    ...
    let data
    try {
        await db.promise().query('SELECT * FROM user ORDER BY bank DESC', [message.guild.id]).then(result => {
            data = result[0]
        })
    } catch (error) {
        console.log(error)
    ...
    .setAuthor(`Global Leaderboard | Bank NightBux`,message.guild.iconURL())
    .setColor("#f44336")
    .setDescription(page.map(e => `<@${e.user_id}> | <:bank:851656706555117588> Bank **${e.bank}**`))
