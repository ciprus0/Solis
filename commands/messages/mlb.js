...
module.exports = {
    name: "mleaderboard",
    aliases:['mlb'],
    description: "view all time message leaderboard",
    utilisation: `{prefix}mlb`,
    category: "Messages",
    module: 'messages',
    ...
    let data
    try {
        await db.promise().query('SELECT * FROM messagecount WHERE guild_id = ? ORDER BY total_messages DESC', [message.guild.id]).then(result => {
            data = result[0]
        })
    } catch (error) {
        console.log(error)
    ...
    .setFooter("You can see other pages with -mlb [pageNbr]")
    )
